...
const CartItem: React.FC<CartItemType> = ({title, image, size, type, count, price, id, index, isBuy = false}) => {
    const dispatch = useDispatch<AppDispatch>();

    return (
        <div className="cart__item">
            <div
                className={`cart__item-img ${isBuy && "cart__item-img_buy"}`}>
                <img
            ...
            <div className={`cart__item-info ${isBuy && "cart__item-info_buy"}`}><h3>{title}</h3><p>{type}, размер: {size}</p></div>
            {
                !isBuy &&
                <>
                    ...
                    <div className="cart__item-remove">
                        <IconButton
                            ...
                            style={{border: '1px solid #fff', width: '32px', height: '32px'}}
                            size="small"
                        >
